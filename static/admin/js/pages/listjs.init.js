...
}, xhttp.open("GET", "/static/json/table-customer-list.json"), xhttp.send();
    ...
    statusVal = (Array.prototype.slice.call(forms).forEach(function (a) {
        a.addEventListener("submit", function (e) {
            ...
            a.checkValidity() ? (e.preventDefault(), "" === customerNameField.value || "" === emailField.value || "" === dateField.value || "" === phoneField.value || editlist ? "" !== customerNameField.value && "" !== emailField.value && "" !== dateField.value && "" !== phoneField.value && editlist && (t = customerList.get({id: idField.value}), Array.from(t).forEach(function (e) {
                ...
                phone: phoneField.value,
                status: isStatus(statusField.value)
            }), customerList.sort("id", {order: "desc"}), document.getElementById("close-modal").click(), refreshCallbacks(), clearFields(), filterContact("All"), count++, Swal.fire({
                position: "center",
                icon: "success",
...
var attroptions = {
        valueNames: ["name", "born", {data: ["id"]}, {attr: "src", name: "image"}, {
            attr: "href",
            name: "link"
        }, {attr: "data-timestamp", name: "timestamp"}]
    }, attrList = new List("users", attroptions), existOptionsList = (attrList.add({
        name: "Leia",
        born: "1954",
        image: "/static/images/users/avatar-5.jpg",
        id: 5,
        timestamp: "67893"
    }), {valueNames: ["contact-name", "contact-message"]}), existList = new List("contact-existing-list", existOptionsList),
